...
+(NSArray*) cheersFromWebService {
    NSURL *url = [NSURL URLWithString:@"http://scoutcheers.azurewebsites.net/api/Cheer"];
    NSData *data = [NSData dataWithContentsOfURL:url];
    NSError *error;
    
    if (!error) {
        NSMutableArray *objects = [NSJSONSerialization JSONObjectWithData:data  options:kNilOptions error:&error];
        
        NSMutableArray *results =  [NSMutableArray arrayWithCapacity:[objects count]];
        
        for (NSMutableDictionary *o in objects) {
            NSString *title = [o objectForKey:@"name"];
            NSString *description = [o objectForKey:@"description"];
            
            [results addObject:[Cheer cheerOfTitle:title description:description]];
        }
